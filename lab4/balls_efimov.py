...
def click(event):
	...
	missed_every_figure = len(figures)
	...
	for figure in figures:
		...
		if (distance <= figure['r']):
			...
			missed_every_figure -= 1
	if missed_every_figure == 0:
		score -= 1
...
# Main game cicle
while not finished and not quited:
	clock.tick(FPS)
	screen.fill(background_color)
	rule1 = f.render('Правила игры:', True, WHITE)
	rule2 = f.render('Вам предстоит нажимать на шарики и квадраты. Кол-во очков', True, WHITE)
	rule3 = f.render('зависит от размера фигур. Промахи снимают по одному очку.', True, WHITE)
	rule4 = f.render('При долгом бездействии шарики растут, при достижении максимального', True, WHITE)
	rule5 = f.render('размера игра заканчивается. Результаты записываются в таблицу.', True, WHITE)
	start = f.render('НАЧАТЬ', True, WHITE)
	if pygame.mouse.get_focused():
		pos = pygame.mouse.get_pos()
		if screen_size[0] // 2 - 450 <= pos[0] <= screen_size[0] // 2 - 330:
			if screen_size[1] // 2 + 20 <= pos[1] <= screen_size[1] // 2 + 81:
				start = f.render('НАЧАТЬ', True, BLACK, WHITE)
	for event in pygame.event.get():
		if event.type == pygame.QUIT:
			...
			if screen_size[0] // 2 - 450 <= pos[0] <= screen_size[0] // 2 - 330:
				if screen_size[1] // 2 + 20 <= pos[1] <= screen_size[1] // 2 + 81:
					finished = True
	screen.blit(rule1, (screen_size[0] // 2 - 450, screen_size[1] // 2 - 285))
	screen.blit(rule2, (screen_size[0] // 2 - 450, screen_size[1] // 2 - 220))
	screen.blit(rule3, (screen_size[0] // 2 - 450, screen_size[1] // 2 - 160))
	screen.blit(rule4, (screen_size[0] // 2 - 450, screen_size[1] // 2 - 100))
	screen.blit(rule5, (screen_size[0] // 2 - 450, screen_size[1] // 2 - 40))
	screen.blit(start, (screen_size[0] // 2 - 450, screen_size[1] // 2 + 20))
	pygame.display.update()
	screen.fill(BLACK)
