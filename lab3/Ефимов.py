...
def draw_polyline(color, point_set):
	...
	for i in range(len(point_set) - 1):
		line(screen, color, point_set[i], point_set[i+1],
                     basic_line_width)
...
def draw_head(color, center, head_size):
	...
	ellipse(screen, color,
                pygame.Rect((int(center[0] - head_size * 0.6),
			     int(center[1] - head_size * 0.5),
			     int(head_size * 1.2),
			     head_size
			)
		),
...
def draw_legs(color, base_point, length, type):
	...
	point_set = [
		...
		(base_point[0] + length / 3, base_point[1] + length * 31 / 30),
		]
...
# Drawing two boys in sides of girls.
draw_boy(-HEIGHT / 4, HEIGHT / 6, arms={'left' : 'straight', 'right' : 'straight'})
draw_boy(HEIGHT / 4, HEIGHT / 6, arms={'left' : 'straight', 'right' : 'straight'})

# Drawing extra boy on the left.
draw_boy(-HEIGHT/6-HEIGHT/4, HEIGHT/6*1.2, arms={'left': 'bend', 'right': 'bend'})

# Drawing icecreams and a balloon (with the function for icecream).
draw_icecream(
...
draw_icecream(
	colors={'cone' : RED, 'ball_1' : RED, 'ball_2' : RED},
	base_point=(center + np.array([-HEIGHT / 3, HEIGHT / 25])),
