        ...
        for body in space_objects:
            for obj in space_objects:
                ...
                planet_data = [str((obj.Vx ** 2 + obj.Vy ** 2) ** (1 / 2)),
                               str(((body.x - obj.x) ** 2 + (body.y - obj.y) ** 2) ** 0.5), str(dt + 1)]
...
def graph():
    ...
    for i in range(len(input_data)):
        speed.append(np.log(float(input_data[i][0])) / np.log(10))
        distance.append(np.log(float(input_data[i][1])) / np.log(10))
        time.append(np.log(float(input_data[i][2])) / np.log(10))
    plt.subplot(221)
    plt.plot(time, speed)
